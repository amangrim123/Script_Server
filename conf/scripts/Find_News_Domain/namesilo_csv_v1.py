...
def send_output_email(send_result):
    ...
    receiver_emails = ["akrha777@gmail.com"]
...
if __name__ == '__main__':
    ...
    Number_of_Scripts_to_run_parallel = 64
    Batch_size = 16
    GoogleUrls_Count = 8
    ...
    domain_file_first = "abcd.csv"

    domain_csv = domain_folder + Path_Slash + domain_file_first

    ##################### Get Input File From ##################

    External_arguments = sys.argv[1]

    argv_f = open(External_arguments,'r')
    argv_file = argv_f.read()
    argv_f.close()

    script_f = open(domain_csv,'w')
    script_f.write(argv_file)
    script_f.close()
